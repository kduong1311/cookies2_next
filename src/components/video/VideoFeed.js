...
export default function VideoFeed({
  ...
  const [allPostIds, setAllPostIds] = useState([]); // Chỉ lưu danh sách ID posts
  const [loadedPosts, setLoadedPosts] = useState([]); // Posts đã được load chi tiết
  const [users, setUsers] = useState({});
  const [currentPostIndex, setCurrentPostIndex] = useState(0);
  const [loading, setLoading] = useState(true);
  ...
  const POSTS_PER_BATCH = 5; // Số lượng posts load mỗi lần
  ...
  // Function để load chi tiết của một batch posts
  const loadPostBatch = async (postList, startIndex, count) => {
    ...
    try {
      // Fetch post details
      const postDetailPromises = postsToLoad.map(async (post) => {
        try {
          const res = await axios.get(`http://103.253.145.7:8080/api/posts/${post.post_id}`, {
            withCredentials: true
          });
      ...
      postDetails.forEach(detail => {
        if (detail.status === 'success') {
          newPostData[detail.data.post_id] = detail.data;
          newLoadedPosts.push(detail.data);
        }
      });

      // Fetch user data
      const userPromises = postsToLoad.map(async (post) => {
        try {
          const res = await axios.get(`http://103.253.145.7:3000/api/users/${post.user_id}`, {
            withCredentials: true
          });
  ...
  // Check nếu cần load thêm posts khi di chuyển
  const checkAndLoadMore = useCallback(async (newIndex) => {
    // Load thêm khi còn 2 posts nữa là hết
    if (newIndex >= loadedPosts.length - 2 && loadedPosts.length < allPostIds.length && !loadingMore) {
        ...
        {loadedPosts.length > 0 && (
          <div className="flex items-center justify-center transition-all duration-300 relative">
            ...
            >
              ...
              {/* Loading indicator khi đang load thêm posts */}
              {loadingMore && (
