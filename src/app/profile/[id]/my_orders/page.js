...
const OrdersPage = () => {
  ...
  const handleOrderClick = async (order) => {
    ...
    // Fetch additional details for products and shops
    const details = { products: {}, shops: {} };
    
    for (const item of order.items) {
      try {
        // Fetch product details
        const productResponse = await fetch(`http://103.253.145.7:3003/api/products/${item.product_id}`);
        ...
        // Fetch shop details
        const shopResponse = await fetch(`http://103.253.145.7:3002/api/shops/${item.shop_id}`);
